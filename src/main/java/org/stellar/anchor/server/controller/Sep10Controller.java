...
public class Sep10Controller {
    ...
    public ChallengeResponse createChallenge(@RequestParam String account,
                                             @RequestParam(required = false) String memo,
                                             @RequestParam(required = false, name = "home_domain") String homeDomain,
                                             @RequestParam(required = false, name = "client_domain") String clientDomain) throws SepException {
        debugF("/auth account={} memo={} home_domain={}, client_domain={}", account, memo, homeDomain, clientDomain);
        ChallengeRequest challengeRequest = ChallengeRequest.of(account, memo, homeDomain, clientDomain);
    ...
    public ValidationResponse validateChallenge(@RequestParam String transaction) throws InvalidSep10ChallengeException, IOException, URISyntaxException, SepValidationException {
    ...
    @RequestMapping(
            ...
            method = {RequestMethod.POST})
    public ValidationResponse validateChallenge(@RequestBody(required = false) ValidationRequest validationRequest) throws InvalidSep10ChallengeException, IOException, URISyntaxException, SepValidationException {
        debug("/auth details:", validationRequest);
        return sep10Service.validateChallenge(validationRequest);
    }


    @ExceptionHandler({SepException.class, SepValidationException.class, InvalidSep10ChallengeException.class, URISyntaxException.class})
