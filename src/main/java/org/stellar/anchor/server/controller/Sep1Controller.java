...
public class Sep1Controller {
    ...
    public Sep1Controller(
            Sep1Config sep1Config,
            Sep1Service sep1Service) {
        this.sep1Config = sep1Config;
        this.sep1Service = sep1Service;
    }

    @CrossOrigin(origins = "*")
    @RequestMapping(value = "/.well-known/stellar.toml", method = {RequestMethod.GET, RequestMethod.OPTIONS})
    public ResponseEntity<String> getToml() throws IOException, SepNotFoundException {
        ...
        return ResponseEntity.ok()
                .headers(headers)
                .body(sep1Service.getStellarToml());
    }
